<template>
  <div class="auth-page">
    <div class="auth-card">
      <h1 class="auth-title">{{ $t('auth.setup.title') }}</h1>
      <p class="auth-subtitle">{{ $t('auth.setup.subtitle') }}</p>
      ...
      <form class="auth-form" @submit.prevent="submit">
        ...
        <div class="field">
          <label for="confirm">{{ $t('auth.setup.confirm') }}</label>
          <input
            id="confirm"
            v-model="confirm"
            type="password"
        ...
        <button type="submit" class="auth-btn" :disabled="loading">
          {{ loading ? $t('auth.setup.loading') : $t('auth.setup.submit') }}
        </button>
      </form>
    </div>
  </div>
...
async function submit() {
  ...
  try {
    ...
    error.value = t('auth.setup.error')
  } finally {
    loading.value = false
  }
}
